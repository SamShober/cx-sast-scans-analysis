...
function getSummary($data, $writeToSummaryTxtFile){
    ...
    $fileCountMetrics = $data | Where-Object -FilterScript {$_.scanStatus -eq "Finished" -and $_.isIncremental -eq "Full"} | Measure-Object -Property fileCount -Maximum -Sum -Average -Minimum
...
function writeDatatoExcelFromCell($data, $sheet, $startCol, $Row, $endCol){
    ...
    $range = $sheet.Range($startCol + $Row,"$endCol$($array.GetLength(0))")
    $range.Value2 = $array
